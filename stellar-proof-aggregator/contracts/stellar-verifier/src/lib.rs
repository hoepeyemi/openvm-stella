...
mod tests {
    ...
    /// Active smoke-test for the verification entrypoint. With soroban-sdk v22,
    /// BN254 host functions are not exposed, so the contract must reach the
    /// explicit unsupported-host-function boundary after ABI and VK validation.
    #[test]
    #[should_panic]
    fn test_verify_aggregate_reports_unsupported_bn254_host_function() {
        let env = Env::default();
        env.mock_all_auths();
        let cid = env.register(AggregatedVerifier, ());
        let client = AggregatedVerifierClient::new(&env, &cid);
        ...
        for _ in 0..8 {
            pub_inputs.push_back(zero_fr(&env));
        }

        client.verify_aggregate(&proof, &10_u32, &pub_inputs);
    }
}
