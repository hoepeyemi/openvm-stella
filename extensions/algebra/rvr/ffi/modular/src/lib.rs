...
impl<F: PrimeField<Repr = [u8; 32]>> KnownFieldArith for KnownPrimeField<F> {
    type Elem = F;
    #[inline(always)]
    unsafe fn read_elem(&self, state: *mut c_void, ptr: u32) -> F {
        read_field_256(state, ptr)
    }
    #[inline(always)]
    unsafe fn write_elem(&self, state: *mut c_void, ptr: u32, val: &F) {
...
impl KnownFieldArith for KnownPrimeField<Bls12381Fq> {
    type Elem = blstrs::Fp;
    #[inline(always)]
    unsafe fn read_elem(&self, state: *mut c_void, ptr: u32) -> blstrs::Fp {
        read_bls12_381_fq(state, ptr)
    }
    #[inline(always)]
    unsafe fn write_elem(&self, state: *mut c_void, ptr: u32, val: &blstrs::Fp) {
